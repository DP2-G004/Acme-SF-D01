...
public class ClientContractPublishService extends AbstractService<Client, Contract> {
	...
	public void authorise() {
		...
		boolean status = contract != null && contract.isDraftMode() && principal.hasRole(contract.getCustomerName()) && contract.getCustomerName().getUserAccount().getId() == userAccountId;
	...
	public void validate(final Contract object) {
		...
		if (!super.getBuffer().getErrors().hasErrors("budget")) {
			...
			final boolean budget = object.getBudget().getAmount() + contracts.stream().mapToDouble(x -> x.getBudget().getAmount()).sum() > object.getProject().getCost().getAmount();
